(function ($) {
    ...
    var PTB_CPT = library(function () {
        ...
        var createMetaBoxItem = function (id, options, ready) {

            var selector = "#" + options.type + "_\\{\\{id\\}\\}";
            if ($(selector).length > 0) {
                var reg = ready ? new RegExp(options.type + '_{{id}}', "gi") : new RegExp('{{id}}', "gi");
                var html = $(selector).clone()[0].outerHTML.replace(reg, ready ? id : options.id),
        ...
        var sanitize_post_type_slug = function ($val) {
            return $val.replace(/[^A-Za-z0-9_]+/g, '-').toLowerCase();
        };

        var sanitize_slug = function ($val) {
