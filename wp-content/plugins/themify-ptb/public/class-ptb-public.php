...
class PTB_Public {
    ...
    public function ptb_filter_cpt_category_archives(&$query) {
        if (!self::$shortcode   && !is_singular() && !$query->is_category() &&  !is_feed($query) && ($query->is_post_type_archive()  || $query->is_tag() || $query->is_tax()) && (!isset($query->query_vars['suppress_filters']) || $query->query_vars['suppress_filters'])) {
            self::$template = false;
    ...
    public function ptb_shortcode($atts) {
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) {
                ...
                $post_meta = array_merge($post_meta, get_post_custom(), (array) $post);
